...
public class IfThenElse extends Stmt {
	...
	public CheckState check(CheckState s) throws Exception {
		...
		if (tipoCondcion != Tipo.Booleano) {
			s.errores.add("La condicion del IfThenElse no es del tipo esperado, " + tipoCondcion);
		}

		CheckState thenBodyState = (CheckState) s.clone();
		CheckState elseBodyState = (CheckState) s.clone();

		// Obtenemos el estado resultante de le interseccion entre el cuerpo del
		// ThenBody y ElseBody.
		CheckState resultState = CheckState.intersect(thenBody.check(thenBodyState), elseBody.check(elseBodyState));

		// Retornamos la interseccion entre el estado Actual y el estado
		// resultante de la
		// interseccion entre el cuerpo ThenBody y ElseBody.
