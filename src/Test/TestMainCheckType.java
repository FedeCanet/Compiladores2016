...
public class TestMainCheckType {
	...
	public void test() {
		...
		// CASO 5 - Asignaci�n de expresiones y Suma:
		lines[0] = "x = 1 + y = 2;";
		state = new CheckState();
		state.create("x", new VarInfo(Tipo.Numerico, true));
		...
		state = new CheckState();
		state.create("var", new VarInfo(Tipo.Booleano, true));
		//el x no va porque se declara en el cuerpo del if y al hacer intersecci�n no se obtiene.
		lines = new String[2];
		...
		//Caso 13 - While-Do con declaraci�n dentro del bucle.
		state = new CheckState();
		state.create("x", new VarInfo(Tipo.Numerico, true));
		state.create("y", new VarInfo(Tipo.Numerico, true));
		//el z no va porque se declara dentro del cuerpo del While, al hacer la intersecci�n no se obtiene.
		lines = new String[3];
		...
		// Caso 14 - Con Errores:
		state = new CheckState();
		//Asumimos que es n�merico, debe pasar el test pero imprimir error.
		state.create("x", new VarInfo(Tipo.Numerico, true));
		lines = new String[2];
	...
	/**
	 * M�todo, recibe una entrada y un estado, verifica que la entrada genere el mismo estado
